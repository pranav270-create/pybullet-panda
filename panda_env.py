...
class PandaEnv():
    def __init__(self,
        ...
        self.visualShapeId2 = None
        self.collisionShapeId2 = None
        self.multiBodyId = None
        self.multiBodyId2 = None
    ...
    def reset_env(self):
        ...
        # Load can on table
        #self.canID = [p.loadURDF('/geometry/cube.urdf', basePosition=[0.5, 0, 0.2])]
        #self.textureID = p.loadTexture('tex256.png')
        #p.changeVisualShape(self.canID[0], 0, textureUniqueId=self.textureID)

        # Load using p.createvisualShape

        # self.visualShapeId = p.createVisualShape(shapeType=p.GEOM_MESH, fileName='geometry/coca-cola.obj', rgbaColor=None, meshScale=[0.02, 0.02, 0.02])
        # self.collisionShapeId = p.createCollisionShape(shapeType=p.GEOM_MESH, fileName='geometry/coca-cola.obj', meshScale=[0.02, 0.02, 0.02])
        # self.multiBodyId = p.createMultiBody(baseMass=0.1, baseCollisionShapeIndex=self.collisionShapeId, baseVisualShapeIndex=self.visualShapeId, basePosition=[0.7, 0, 0.2], baseOrientation=p.getQuaternionFromEuler([0, 0, 0]))
        # self.textureID = p.loadTexture('princeton-2.jpg')
        # p.changeVisualShape(self.multiBodyId, -1, textureUniqueId=self.textureID)

        self.visualShapeId2 = p.createVisualShape(shapeType=p.GEOM_MESH, fileName='geometry/hollow_cylinder.obj', rgbaColor=None, meshScale=[0.02, 0.02, 0.02])
        self.collisionShapeId2 = p.createCollisionShape(shapeType=p.GEOM_MESH, fileName='geometry/hollow_cylinder.obj', meshScale=[0.02, 0.02, 0.02])
        self.multiBodyId2 = p.createMultiBody(baseMass=0.1, baseCollisionShapeIndex=self.collisionShapeId2, baseVisualShapeIndex=self.visualShapeId2, basePosition=[0.7, 0, 0.2], baseOrientation=p.getQuaternionFromEuler([0, 0, 0]))
        #p.changeVisualShape(self.multiBodyId2, -1, textureUniqueId=self.textureID)
